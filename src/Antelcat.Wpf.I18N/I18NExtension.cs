...
{
	internal static readonly IDictionary<string, object?> Target;
		...
		foreach (var bindingBase in Keys)
		{
			switch (bindingBase)
			{
				...
				case Binding propBinding:
					...
					break;
				default:
					throw new ArgumentException($"{nameof(Keys)} only accept {typeof(LanguageBinding)} or {typeof(Binding)} current type is {bindingBase.GetType()}");
			}
	...
	/// <summary>
	/// You know why, am I right? <see cref="Collection{BindingBase}"/>
	/// </summary>
